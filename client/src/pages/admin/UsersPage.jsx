...
export const AdminUsersPage = () => {
  ...
  const handleStatusToggle = async (user) => {
    const nextStatus =
      getNormalizedStatus(user) === "active" ? "suspended" : "active";
  ...
  const handleSaveAreas = async (user) => {
    try {
      ...
      setSuccessMessage("Contractor assigned areas updated.");
      loadUsers();
    } catch (err) {
      const message =
        err.response?.data?.error ||
        err.response?.data?.message ||
        "Failed to update contractor areas";
      setError(message);
    } finally {
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              >
                ...
                <option value="OPERATOR">Contractor</option>
                ...
                {filteredUsers.map((user) =>
                  (() => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-neutral-900 dark:text-slate-200">
                          {user.name}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${roleBadgeClass[user.role] || roleBadgeClass.CITIZEN}`}
                          >
